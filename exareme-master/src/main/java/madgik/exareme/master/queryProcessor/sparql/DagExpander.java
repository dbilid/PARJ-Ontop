...
public class DagExpander {
			...
			if (!op.isExpanded()) {
				...
				if (op.getObject() instanceof NonUnaryWhereCondition && op.isCommutativity()) {
					...
					if (bwc.getOperator().equals("=")) {
						...
						if (useCommutativity && op.getChildren().size() == 2) {
					...
					if (bwc.getOperator().equals("=")) {
						...
						if (op.getChildren().size() > 1) {
							...
							for (Node c3 : c2.getChildren()) {
								// boolean childrenBase =
								// c3.getDescendantBaseTables().size() == 2
								...
								if (c3.getObject() instanceof NonUnaryWhereCondition) {
									...
									if (bwc2.getOperator().equals("=")) {
										...
										boolean addImplicit = op.getChildren().size() == 2
												&& c3.getChildren().size() == 2;
										...
										// }
										if (c3.getChildren().size() == 2
												&& c3.getChildAt(1).getDescendantBaseTables().size() > 1) {
										...
										if (comesFromLeftOp) {
											...
											if (!hashes.containsKey(associativityTop.computeHashIDExpand())
													|| hashes.get(associativityTop.computeHashIDExpand()).getParents()
															.isEmpty()) {
												...
												if (associativity.getParents().isEmpty()) {
													if (hashes.get(
															associativity.computeHashIDExpand()) == associativity) {
											...
											// add implicit (A join_{a=b} B)
											// join_{a=c} C -> (A join_{a=b} B)
											// join_{b=c} C
											if (addImplicit && bwc.getLeftOp().equals(bwc2.getLeftOp())) {
												//System.out.println("adding");
												Node associativityImplicit = new Node(Node.AND, Node.JOIN);
												NonUnaryWhereCondition bwcImplicit = new NonUnaryWhereCondition();
												bwcImplicit.setOperator("=");
												bwcImplicit.setRightOp(bwc.getRightOp());
												bwcImplicit.setLeftOp(bwc2.getRightOp());

												// bwcImplicit.addRangeFilters(bwc2);
												// newBwc2.setLeftOp(bwc.getRightOp());
												associativityImplicit.setObject(bwcImplicit);
												associativityImplicit.addChild(c2);
												associativityImplicit.addChild(op.getChildAt(1));

												// System.out.println(associativityTop.getObject().toString());
												if (!hashes.containsKey(associativityImplicit.computeHashIDExpand())
														|| hashes.get(associativityImplicit.computeHashIDExpand())
																.getParents().isEmpty()) {
													hashes.put(associativityImplicit.computeHashIDExpand(),
															associativityImplicit);
													// Node newTop =
													// hashes.checkAndPutWithChildren(associativityTop);
													// hashes.remove(eq.computeHashIDExpand());
													// for (Node p :
													...
													// hashes.remove(p.computeHashIDExpand());
													// }
													// associativityImplicit.setSwap(false);
													eq.addChild(associativityImplicit);
													associativityImplicit
															.addAllDescendantBaseTables(op.getDescendantBaseTables());

												} else {

													unify(eq, hashes.get(associativityImplicit.computeHashIDExpand())
															.getFirstParent());
													// same as unify(eq', eq)???
													// checking again children
													// of eq?
													associativityImplicit.removeAllChildren();
													// hashes.get(associativityImplicit.computeHashIDExpand()).setSwap(false);
													if (c2.getParents().isEmpty()) {
														if (hashes.get(c2.computeHashIDExpand()) == c2) {
															hashes.remove(c2.computeHashIDExpand());
														}
											...
											if (addImplicit && bwc.getLeftOp().equals(bwc2.getLeftOp())) {
												...
												bwcImplicit.setOperator("=");
												bwcImplicit.setRightOp(bwc.getRightOp());
												bwcImplicit.setLeftOp(bwc2.getLeftOp());
								...
								if (c3.getObject() instanceof NonUnaryWhereCondition && c3.getChildren().size() > 1) {
									...
									if (bwc2.getOperator().equals("=")) {
										...
										if (comesFromLeftOp2 || comesFromRightOp2) {
											...
											NonUnaryWhereCondition newBwc = new NonUnaryWhereCondition();
											newBwc.setOperator("=");
											Node associativityNew = new Node(Node.AND, Node.JOIN);
											newBwc.setRightOp(bwc.getRightOp());
											newBwc.setLeftOp(bwc.getLeftOp());
											newBwc.addRangeFilters(bwc);
											associativityNew.setObject(newBwc);

											associativityNew.addChild(c3.getChildAt(0));

											Node table = new Node(Node.OR);

											if (hashes.containsKey(associativityNew.computeHashIDExpand())
													&& !hashes.get(associativityNew.computeHashIDExpand()).getParents()
															.isEmpty()) {
												Node assocInHashes = hashes.get(associativityNew.computeHashIDExpand());
												table = assocInHashes.getFirstParent();

												associativityNew.removeAllChildren();
												// associativity =
												// assocInHashes;

											} else {
												hashes.put(associativityNew.computeHashIDExpand(), associativityNew);
												table.addChild(associativityNew);

												// table.setPartitionedOn(new
												// PartitionCols(newBwc.getAllColumnRefs()));
												hashes.put(table.computeHashIDExpand(), table);

												associativityNew.addAllDescendantBaseTables(
														c3.getChildAt(0).getDescendantBaseTables());

												table.addAllDescendantBaseTables(
														associativityNew.getDescendantBaseTables());
											}
	...
	public SinglePlan getBestPlanCentralized(Node e, double limit, Memo memo) {
		...
		if (memo.containsMemoKey(ec) && memo.getMemoValue(ec).isMaterialised()) {
			...
			// CentralizedMemoValue cmv = (CentralizedMemoValue)
			// memo.getMemoValue(ec);
	...
	private SinglePlan searchForBestPlanCentralized(Node e, double limit, Memo memo) {
		...
		for (int k = 0; k < e.getChildren().size(); k++) {
			...
			for (int i = 0; i < o.getChildren().size(); i++) {
				...
				// CentralizedMemoValue cmv = (CentralizedMemoValue)
				// memo.getMemoValue(new MemoKey(e2, null));
